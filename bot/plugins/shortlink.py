...
# ── /set_admins — manage admin list from Telegram ─────────────────────────────
@Client.on_message(filters.command('set_admins'))
async def manage_admins(bot, message):
    if not await _admin_check(message): return
    import info as _info
    from utils import _schedule_save

    parts = message.text.strip().split(None, 2)
    action = parts[1].lower().strip() if len(parts) >= 2 else ''

    if action == 'list' or not action:
        current = '\n'.join(f'• `{uid}`' for uid in _info.ADMINS) or '_None_'
        usage = (
            '\n\n**Commands:**\n'
            '`/set_admins add <user_id>` — grant admin\n'
            '`/set_admins remove <user_id>` — revoke admin\n'
            '`/set_admins list` — show all admins'
        ) if not action else ''
        return await message.reply(
            f'👮 **Admins ({len(_info.ADMINS)}):**\n\n{current}{usage}',
            parse_mode='markdown'
        )
    ...
    if action not in ('add', 'remove') or len(parts) < 3:
        return await message.reply(
            '**Usage:**\n'
            '`/set_admins add <user_id>`\n'
            '`/set_admins remove <user_id>`\n'
            '`/set_admins list`',
            parse_mode='markdown'
        )
    ...
    if action == 'add':
        if target_id in _info.ADMINS:
            return await message.reply(
                f'⚠️ `{target_id}` is already an admin.',
                parse_mode='markdown'
            )
        _info.ADMINS.append(target_id)
        _schedule_save()
        await message.reply(
        ...
        if target_id not in _info.ADMINS:
            return await message.reply(
                f'⚠️ `{target_id}` is not in the admin list.',
                parse_mode='markdown'
            )
        caller_id = message.from_user.id if message.from_user else None
        if target_id == caller_id and len(_info.ADMINS) <= 1:
            return await message.reply(
                '❌ Cannot remove yourself — you are the only admin!\n'
                'Add another admin first: `/set_admins add <user_id>`',
                parse_mode='markdown'
            )
        _info.ADMINS.remove(target_id)
        _schedule_save()
        await message.reply(
